...
class _FundingPageState extends State<FundingPage>
    with AutomaticKeepAliveClientMixin<FundingPage> {
  @override
  bool get wantKeepAlive {
    return BlocProvider.of<TabsCubit>(context).state.activeTabs.contains(2);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator.adaptive(
      onRefresh: () => context.read<FundingNewsCubit>().loadFundingNews(),
      color: AppColors.primaryColor,
      child: BlocBuilder<FundingNewsCubit, FundingNewsState>(
        builder: (context, state) {
          if (state is FundingNewsLoading) {
            return const Center(child: CircularProgressIndicator());
          }

          if (state is FundingNewsLoaded) {
                  ...
                  IconButton(
                      onPressed: () {
                        if (SingleTapDetector.isClicked == false) {
                          ...
                          context.read<FundingNewsCubit>().loadFundingNews();
                        }
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      if (state.message.toLowerCase().contains("empty")) ...[
                        Text(AppTexts.noNewsDescriptionFunding),
                      ]
                    ],
                  )
                ],
